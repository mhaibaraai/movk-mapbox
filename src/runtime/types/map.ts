...
export interface MaplibreContext {
  ...
  /** 当前样式自带的图层 id（每次 style.load 时快照），不含运行时由组件添加的图层 */
  styleLayerIds: Readonly<ShallowRef<readonly string[]>>
  /** 地图实例创建后 resolve；Marker、Popup、控件、相机等无需等待瓦片加载的操作用它 */
  whenAttached: () => Promise<MaplibreMap>
  /** 样式首次加载完成时 resolve */
