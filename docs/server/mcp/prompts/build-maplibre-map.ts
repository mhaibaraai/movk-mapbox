...
export default defineMcpPrompt({
  description: 'Assemble a declarative @movk/maplibre map for a described task, using only documented components and following the library conventions.',
  inputSchema: {
  ...
  async handler({ task, framework }) {
    ...
    const text = [
      `Build a self-contained \`<MaplibreMap>\` single-file component for ${target} that accomplishes this task:`,
      ``,
      `"${task}"`,
      ``,
      `Requirements:`,
      `- Use ONLY components that exist in the candidate list below. Do not invent component names.`,
      `- Follow the library conventions in the \`resource://docs/maplibre-conventions\` resource (SSR-safe, explicit container height, camera v-model diffing, omitUndefined, onReady/watch/onUnmounted layer lifecycle).`,
      `- Give the map container an explicit height (e.g. class="h-115").`,
      `- For exact props, call the \`list-maplibre-components\` and \`get-component\` tools; for layer paint/layout keys call \`get-layer-paint-schema\`.`,
      ``,
