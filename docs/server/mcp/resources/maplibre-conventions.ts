...
// Hard constraints for writing correct declarative @movk/maplibre code.
// Kept in sync with the Skill's references/conventions.md.
const conventions: Convention[] = [
  {
    topic: 'SSR safety',
    rule: 'Use <MaplibreMap> and child components directly; no <ClientOnly> wrapper is needed.',
    why: 'The map instance is created only on the client in onMounted; components are already SSR-safe.'
  },
  {
    topic: 'Container height',
    rule: 'MaplibreMap fills width:100%;height:100%, so the parent must have an explicit height (e.g. class="h-115"), otherwise the map is invisible.',
    why: 'With no height the container collapses to 0 and the map does not render.'
  },
  {
    topic: 'Strip undefined from options',
    rule: 'Run option objects through omitUndefined (@movk/core) before passing them to the maplibre API.',
    why: 'maplibre checks fields with `key in options`; a leftover undefined pollutes the camera matrix (+undefined → NaN).'
  },
  {
  ...
  },
  {
    ...
    rule: 'Inject MapLibre / Tianditu tokens via runtimeConfig.public.maplibre instead of hardcoding; prefer environment variables.',
...
export default defineMcpResource({
  uri: 'resource://docs/maplibre-conventions',
  description: 'Key conventions and gotchas for writing correct declarative @movk/maplibre code (SSR safety, container height, omitUndefined, camera v-model, layer lifecycle, unique component names, token injection).',
  cache: '1h',
