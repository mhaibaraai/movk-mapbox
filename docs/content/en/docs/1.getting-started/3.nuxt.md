...
seo:
  title: Nuxt 4
  description: Register the @movk/maplibre module in nuxt.config.ts and configure tiandituToken, glyphs and textFont; SSR-safe with no ClientOnly required.
---
...
| --- | --- | --- |
| `tiandituToken` | `NUXT_PUBLIC_MAPLIBRE_TIANDITU_TOKEN` | Tianditu service `tk` (required when using `MaplibreTiandituLayer`) |
