...
seo:
  title: MCP Server
  description: Connect AI assistants to the built-in Movk MapLibre MCP server to search components, composables and examples, plus custom tools for coordinate conversion and layer paint schemas.
---
...
The server provides structured access to this library so AI tools can more accurately understand and assist with `@movk/maplibre` development. The endpoint is `https://maplibre.mhaibaraai.cn/mcp`.
...
- **`resource://docs/maplibre-conventions`**: Hard constraints for the library's declarative patterns (SSR, container height, `omitUndefined`, camera v-model, layer lifecycle, etc.)
- **`resource://docs/basemap-styles`**: Available basemap presets (keyless OpenFreeMap vector styles and Tianditu layer types: vec/img/ter)
...
- **`list-maplibre-components`**: List all capabilities by category (core / layers / controls / effects / environment / extensions / composables / utils), with optional category or keyword filtering
...
- **`build-maplibre-map`**: Assemble a map from a task description, using only documented components and following library conventions

In tools that support prompts, invoke them with `/`.

## Configuration

The MCP Server uses HTTP transport and can be connected from various AI tools. The name is `movk-maplibre` and the URL is `https://maplibre.mhaibaraai.cn/mcp`.
...
```bash [Terminal]
claude mcp add --transport http movk-maplibre https://maplibre.mhaibaraai.cn/mcp
```
...
```json [claude_desktop_config.json]
{
  "mcpServers": {
    "movk-maplibre": {
      ...
      "args": [
        "mcp-remote",
        "https://maplibre.mhaibaraai.cn/mcp"
      ]
    }
  }
}
...
::install-button
---
url: "https://maplibre.mhaibaraai.cn/mcp"
---
::
...
3. Create a new connector in the Connectors tab: name `Movk MapLibre`, MCP server URL `https://maplibre.mhaibaraai.cn/mcp`, authentication `None`
4. Click Create
...
- "Get the props for MaplibreLayer"
...
- "Use build-maplibre-map to build a map with a Tianditu vector basemap and a navigation control"
...
::tip
Use this library's MCP for wrapper components and conventions; for native MapLibre GL API and style spec questions, see the [MapLibre GL JS docs](https://maplibre.org/maplibre-gl-js/docs/) and the [MapLibre Style Spec](https://maplibre.org/maplibre-style-spec/).
::
