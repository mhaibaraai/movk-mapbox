---
title: transformPoint / transformGeoJSON
description: Convert coordinate points and GeoJSON between WGS84, GCJ02, BD09 and EPSG3857 using gcoord.
category: coordinate
seo:
  title: transformPoint / transformGeoJSON
  description: Convert points and GeoJSON between WGS84, GCJ02, BD09 and EPSG3857 with transformPoint and transformGeoJSON, powered by gcoord.
links:
  - label: gcoord
    icon: i-simple-icons-github
    to: https://github.com/hujiulong/gcoord
    target: _blank
---

## Introduction

`transformPoint` / `transformGeoJSON` convert coordinates between WGS84, GCJ02, BD09 and EPSG3857 using [gcoord](https://github.com/hujiulong/gcoord): the former converts a single longitude/latitude point, the latter converts any GeoJSON and returns a new object (without mutating the input). Commonly used to correct datum offsets in Amap / Tencent (GCJ02) or Baidu (BD09) data back to WGS84, aligning Tianditu and MapLibre base maps.

Both are imported from `@movk/maplibre/utils/coordinate` (not auto-imported):

```ts
import { transformGeoJSON, transformPoint } from '@movk/maplibre/utils/coordinate'

const gcj02 = transformPoint([121.4737, 31.2304], 'WGS84', 'GCJ02')
const wgs84 = transformGeoJSON(raw, 'GCJ02', 'WGS84', { precision: 6 })
```

::note{to="/docs/extensions/coordinate"}
Full visual examples (point correction, GeoJSON correction, precision control) are available on the **Coordinate Conversion** extension page.
::

## API

### `transformPoint()`{lang="ts-type"}

Convert a single longitude/latitude point and return the new coordinates.

::field-group
  ::field{name="point" type="[number, number]" required}
  Longitude/latitude point `[lng, lat]`.
  ::
  ::field{name="from" type="CRS" required}
  Source coordinate reference system.
  ::
  ::field{name="to" type="CRS" required}
  Target coordinate reference system.
  ::
  ::field{name="options.precision" type="number"}
  Number of decimal places to keep in the output coordinates. Omit to retain full precision.
  ::
::

Returns `[number, number]`: the converted `[lng, lat]`.

### `transformGeoJSON()`{lang="ts-type"}

Convert any GeoJSON (point / line / polygon / collection) and return a new object of the same type as the input, without mutating the original.

::field-group
  ::field{name="geojson" type="T" required}
  Any GeoJSON object (`Feature` / `Geometry` / `FeatureCollection`, etc.).
  ::
  ::field{name="from" type="CRS" required}
  Source coordinate reference system.
  ::
  ::field{name="to" type="CRS" required}
  Target coordinate reference system.
  ::
  ::field{name="options.precision" type="number"}
  Number of decimal places to keep in the output coordinates. Omit to retain full precision.
  ::
::

Returns `T`: a new GeoJSON object of the same type as the input.

### `CRS`{lang="ts-type"}

Coordinate reference system identifier: `'WGS84'` | `'GCJ02'` | `'BD09'` | `'EPSG3857'`.

## Changelog

:commit-changelog{prefix="utils" suffix="ts"}
