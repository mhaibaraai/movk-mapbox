...
```ts
import { createTianditu } from '@movk/maplibre/utils/tianditu-client'
...
For the Tianditu basemap/annotation component, see **MaplibreTiandituLayer**; this page covers pure server-side Web service capabilities and doesn't involve any map component.
::
...
Administrative divisions: query center, boundary and sub-divisions by name (or GB code). The Tianditu WKT `MULTIPOLYGON` (real provinces/cities often contain exclave rings) is parsed into a GeoJSON `MultiPolygon` and converted to WGS84, ready for `MaplibreLayer`.
