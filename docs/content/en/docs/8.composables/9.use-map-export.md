...
::note
When used outside a `<MaplibreMap>` subtree, specify the target map via `options.mapId`. Exporting maps that include third-party tiles may be subject to cross-origin canvas taint restrictions.
::
