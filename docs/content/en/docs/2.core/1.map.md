...
`MaplibreMap` is the root of everything: it creates the `maplibre-gl` instance on the client in `onMounted`, distributes a [MaplibreContext](/docs/composables/use-map#api) via `provide`, and child components access it through `useMap()`. The container is sized at `100%` width and height — make sure the parent has an explicit height (examples use `h-115` throughout).
