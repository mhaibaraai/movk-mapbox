...
::note
`tk` 是浏览器端类型 key（Referer 校验），仅用于天地图底图瓦片。天地图检索/地理编码/行政区划/路线等 Web 服务是服务端专用接口（IP 白名单校验），不能通过 `vue-plugin` 注入，也不能放进 `VITE_` 前缀的环境变量（会被打进客户端 bundle）；需要在自建的服务端（Nuxt Nitro、Vite dev 中间件等）里用 `createTianditu` 调用，详见 [天地图 Web 服务](/docs/utils/tianditu)。
::
