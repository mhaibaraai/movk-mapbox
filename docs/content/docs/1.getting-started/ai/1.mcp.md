...
服务为本组件库提供结构化访问，让 AI 工具更准确地理解并协助 `@movk/maplibre` 开发，访问地址为 `https://maplibre.mhaibaraai.cn/mcp`。
...
- **`resource://docs/maplibre-conventions`**：本库声明式范式的硬约束（SSR、容器高度、`omitUndefined`、相机 v-model、图层生命周期等）
- **`resource://docs/basemap-styles`**：可用底图预设（无需 key 的 OpenFreeMap 矢量样式与天地图图层类型 vec/img/ter）
...
- **`list-maplibre-components`**：按分类（core / layers / controls / effects / environment / extensions / composables / utils）列出全部能力，可按分类或关键词过滤
...
- **`build-maplibre-map`**：按任务描述组装一张地图，仅用文档中存在的组件并遵守本库约定

在支持 Prompt 的工具中可用 `/` 调用。

## 配置

MCP Server 采用 HTTP 传输，可在不同 AI 工具中接入，统一使用名称 `movk-maplibre` 与地址 `https://maplibre.mhaibaraai.cn/mcp`。
...
2. 打开 ChatGPT 设置
3. 在 Connectors 选项卡创建新连接器：名称 `Movk MapLibre`、MCP server URL `https://maplibre.mhaibaraai.cn/mcp`、认证 `None`
4. 点击创建
...
- 「按分类列出所有图层组件」
- 「获取 MaplibreLayer 的 Props」
- 「把这组 WGS84 坐标转成 GCJ02」
- 「circle 图层有哪些常用 paint 属性」
- 「找出与绘制相关的能力」
- 「按 build-maplibre-map 帮我搭一张叠天地图矢量底图、带导航控件的地图」
...
::tip
本库 MCP 查封装组件与约定；MapLibre GL 原生 API 与 style spec 请查阅 [MapLibre GL JS 文档](https://maplibre.org/maplibre-gl-js/docs/) 与 [MapLibre Style Spec](https://maplibre.org/maplibre-style-spec/)。
::
