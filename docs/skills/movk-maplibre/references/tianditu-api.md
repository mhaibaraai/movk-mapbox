...
>
> **坐标系**：天地图各接口的经纬度均为 WGS84（CGCS2000），**不是**高德/腾讯那种加密的 GCJ02（火星坐标）。天地图坐标不需要、也不应该再经过 GCJ02→WGS84 转换——这会凭空引入 50m~1km 量级的偏移。曾经错误地对本包全部四个封装做过这层转换，导致 MapLibre（WGS84）地图上点位明显偏移，已修复；新增封装时不要重新引入该转换。
