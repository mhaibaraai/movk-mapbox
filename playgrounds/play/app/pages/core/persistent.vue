...
// 经 useMaplibre(id) 在组件树之外访问已挂载实例（逃生口）
function flyRandom() {
  const ctx = useMaplibre(mapId)
  ctx?.map.value?.flyTo({
...
<template>
  <MapShowcase
    title="跨路由持久化"
    description="persistent + map-id：配合 <KeepAlive> 切换显隐时复用同一实例，不重建。useMaplibre(id) 可在树外访问。"
    height="420px"
  >
    <template #toolbar>
    ...
    <KeepAlive>
      <MaplibreMap
        ...
        :map-id="mapId"
        persistent
        :options="{ style: 'https://tiles.openfreemap.org/styles/positron', center: [116.39, 39.91], zoom: 10 }"
      />
    </KeepAlive>
