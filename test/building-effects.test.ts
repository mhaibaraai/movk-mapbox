...
describe('buildingGradientPaint', () => {
  it('默认按高度插值,断点单调且读取 render_height', () => {
    ...
    expect(JSON.stringify(colorExpr)).toContain('"get","render_height"')
  ...
  it('支持自定义高度字段', () => {
    const paint = buildingGradientPaint({ heightProperty: 'height', baseProperty: 'min_height' })
    expect(JSON.stringify(paint['fill-extrusion-color'])).toContain('"get","height"')
